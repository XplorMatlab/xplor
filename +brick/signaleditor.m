classdef signaleditor < hgsetget
    ...
    methods
        function E = signaleditor(varargin)
            ...
            if isempty(E.callback) && ~isempty(E.hf_created)
                ...
                brick.controlpositions(E.ok_button, E.hf_created, [1 0], [-40 10 30 20])
            end
        end
        function delete(E)
            brick.delete_valid(E.hf_created)
        end
    end
    ...
    % User brick.interface for manual edition
    methods (Access = 'private')
        ...
        function move_point(E,idx)
            ...
            function update()
                p = get(E.ha,'CurrentPoint'); p = p(1,1:2);
                if idx > 1 && idx < E.npoint
                    E.ik(idx) = brick.coerce(p(1),i_min,i_max);
                end
                E.xk(idx) = brick.coerce(p(2), x_min, x_max);
                %disp([E.ik; E.xk])
                E.update_display()
                if ~isempty(E.callback) && E.do_callback_when_move
                    E.callback(E.x)
