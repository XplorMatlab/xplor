function cubemesh(data,fname)
% function cube(data[,fname])
%---
...
[nx ny nz] = size(data);
if nargin<2, fname = brick.savefile('Select file name for saving mesh and texture'); end
...
brick.savemesh([fname '.tri'],vertices,faces)
brick.savetexture(colors,[fname '.tex'])
...
% if nmax>thr
%     bin = ceil(nmax/thr);
%     data = brick.bin(data,[bin bin bin]);
% end
% [nx ny nz] = size(data);
%
% % Basic coordinates
...
%     faces = [faces(:,[1 2 3]); faces(:,[3 4 1])];
%     brick.savemesh([fname '.tri'],vertices,faces)
%     brick.savetexture(text,[fname '.tex'])
% else
