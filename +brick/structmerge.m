function s = structmerge(s,varargin)
% function s = structmerge(s,s1[,'skip|strict'][,'recursive'][,'type'][,'i'])
%---
...
%   not merely replaced, but are also merged using brick.structmerge
...
if any(size(s1)~=size(s))
    if isempty(s)
        s = repmat(brick.structinit(s),size(s1));
    elseif isscalar(s)
        s = repmat(s,size(s1));
    elseif isscalar(s1)
        s1 = repmat(s1,size(s));
...
for k=1:length(F1)
    ...
    elseif recursive && isfield(s,f) && isstruct(s(1).(f))
        for i=1:numel(s)
            ...
            val = s1(j).(f1);
            if isstruct(val)
                s(i).(f) = brick.structmerge(s(i).(f),val,varargin{:});
            else
