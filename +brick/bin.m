function [data, maskbin] = bin(data,bins,varargin)
% function data = bin(data,bins(vector)|xybin(scalar)[,fun][,'same'][,'mask',mask])
% function [data maskbin] = bin(data,xybin(scalar)[,'mask',mask])
%---
...
%               this new mask 
%
% See also brick.enlarge

% Thomas Deneux
% Copyright 2010-2017

if nargin==0, help brick.bin, return, end
...
if domask
    ...
    maskbin = brick.bin(mask,bins,varargin{:});
    data = brick.imvect(brick.bin(brick.imvect(data,mask),bins,varargin{:}),maskbin);
    return
end
