function hl = gridplot(varargin)
% function hl = gridplot([t,]data[,flag][,steps][,'num']
...
% - steps   [xstep ystep], {xstep ystep}, or ystep (see brick.eegplot)
...
while i<length(varargin)
    ...
    elseif ~isempty(regexpi(a,'^(-|row|col|grid|c)*$'))
        ...
        organize = [tokens{:}];
        coloridx = brick.find('c',organize);
        if ~isempty(coloridx)
...
x = repmat(brick.column(tt),[1 s(2:end)]);
...
if isempty(y)
    ...
    if ~isempty(color), brick.set(hl,'color',color(:)), end
    if ~isempty(plotopt)
        set(hl,plotopt{:})
    end
    if ~isempty(callback)
        for i=1:prod(s(2:1+nd))
            idx = brick.indices(s(2:1+nd),i);
