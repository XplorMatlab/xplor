...
% difference, or the maximal difference max(abs(brick.row(x-y))), or which fields
...
if ~isequal(size(x),size(y))
    fprintf('size of %s is [%s], size of %s is [%s]\n', ...
        xname,brick.strcat(size(x),' '),yname,brick.strcat(size(y),' '))
    return
end
...
elseif isstruct(x) && isscalar(x)
    ...
    F = setdiff(Fx,Fy);
    if ~isempty(F)
        fprintf('Some fields are present in %s but not %s: %s\n',xname,yname,brick.strcat(F,', '))
        ok = false;
    end
    F = setdiff(Fy,Fx);
    if ~isempty(F)
        fprintf('Some fields are present in %s but not %s: %s\n',yname,xname,brick.strcat(F,', '))
        ok = false;
    end
    if ok && ~isequal(Fx,Fy)
    ...
    F = intersect(Fx,Fy); 
    b = brick.map(F,@(f)~isequaln(x.(f),y.(f)),'array');
    if any(b)
        fprintf('%s and %s differ for field(s) %s\n',yname,xname,brick.strcat(F(b),', '))
    elseif ok
