function data=enlarge(data,varargin)
% function data=enlarge(data,fact)
% function data=enlarge(data,size[,outsideval])
% function data=enlarge(data,fact,size[,outsideval])
%---
...
if nargin==0, help brick.enlarge, return, end
...
if length(varargin)>=2 && ~isscalar(varargin{2})
    ...
    if length(varargin)<3
        outsideval = brick.switch_case(islogical(data),false,NaN);
    else
...
elseif isempty(data)
    ...
    data = reshape(data,brick.interleave(2,ones(1,ndim),oldsize));
    data = repmat(data,brick.interleave(2,fact,ones(1,ndim)));
