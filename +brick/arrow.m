function hl = arrow(xdata,ydata,varargin)
%ARROW Draw an arrow
%---
% function [hl =] arrow(xdata,ydata[,tiplength[,tipangle[,thickness]]][,'simple|double'][,'line|patch'][,parname1,value1,...])
%---
% Draw an arrow
% 
% Input:
...
lineax = [xdata(:)'; ydata(:)'];
linepx = brick.coordinates(ha,'a2b',lineax,'position');
% create the arrow
a = linepx(:,1);
b = linepx(:,2);
u = tiplengthrel*(b-a);
...
arrowax = brick.coordinates(ha,'b2a',arrowpx,'position');
...
if ~isempty(opt), brick.set(hl,opt{:}), end
