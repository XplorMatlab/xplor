function [pos cmd] = framedesign(grob,pos,resetflag)
%FRAMEDESIGN Utility to let user reposition graphic objects inside a figure
%---
% function [pos cmd] = framedesign([grob[,pos|cmd[,resetflag]]])
%---
...
dosavefile = false;
if nargin<2
    if isappdata(hf,'brick_framedesign')
        info = getappdata(hf,'brick_framedesign');
        grob = info.grob;
        fname = info.fname;
        dosavefile = true;
        pos = brick.loadvar(fname);
    else
        pos = [];
    end
elseif ischar(pos)
    ...
    if ~isempty(fm)
        fname = [brick.fileparts(fm,'noext') '.mat'];
    else
        ext = brick.fileparts(pos,'ext');
        if ~isempty(ext) && ~strcmp(ext,'.mat'), error 'file must have a .mat extension', end
        fname = [brick.fileparts(pos,'noext') '.mat'];
    end
    dosavefile = true;
    if exist(fname,'file')
        pos = brick.loadvar(fname);
...
state.units = brick.get(obj,'units');
newgrob = true;
if nargin>1
    ...
    for k=1:length(F)
        ...
        if isfield(pos,f)
            ...
            %                 brick.setfigsize(grob.hf,pos.hf(3:4))
            %                 continue
            %             end
...
if ~resetflag
    brick.set(obj,'units',state.units) % restore 'units' properties in case they have been modified
    if dosavefile 
        if ~exist(fname,'file') 
            if brick.dodebug, disp 'i don''t understand when this can happen (i.e. resetflag=false but file not existing)', keyboard, end %#ok<DUALC>
            brick.savevar(fname,pos)
        end
...
%hc=cell2mat(brick.get(obj,'children')); hc = hc(:)';
hc=findall(obj); hc = hc(:)';
hctype = brick.get(hc,'type');
hc(brick.ismemberstr(hctype, ...
...
state.objprop = brick.get(obj,{'buttondownfcn' 'visible' 'units'},'struct');
state.childprop = brick.get(state.children,'hittest','struct');
type = brick.get(obj,'type');
state.figures = obj(brick.ismemberstr(type,'figure'));
state.figprop = brick.get(state.figures,{'resize' 'resizefcn' 'windowbuttondownfcn'},'struct');
state.axes = obj(brick.ismemberstr(type,'axes'));
state.axesprop = brick.get(state.axes,{'DataAspectRatio' 'DataAspectRatioMode' 'Box'},'struct');
state.uicontrols = state.allobj(ismember(get(state.allobj,'type'),{'uicontrol' 'uitable'}));
state.uicontrolprop = brick.get(state.uicontrols,{'enable','buttondownfcn'},'struct');
state.panels = state.allobj(strcmp(get(state.allobj,'type'),'uipanel'));
state.panelprop = brick.get(state.panels,{'bordertype' 'borderwidth'},'struct');
...
info.scale = brick.pointer(info.scales(scalek));
...
brick.set(state.children,'hittest','off')
...
brick.set(obj,state.objprop)
brick.set(obj,'units',state.units) % restore 'units' properties in case they have been modified
brick.set(state.children,state.childprop)
brick.set(state.figures,state.figprop)
...
% end
brick.set(state.axes,state.axesprop)
brick.set(state.uicontrols,state.uicontrolprop)
brick.set(state.panels,state.panelprop)
...
if (nargout==0 && ~dosavefile) || nargout==2
    ...
    for k=1:length(F)
        ...
        cmdk = ['pos.' f ' = ['];
        for i=1:nk
            cmdk = [cmdk brick.strcat(round(pos.(f)(i,:)),' ')]; %#ok<*AGROW>
...
switch get(hf,'SelectionType')
    case 'normal'
        ...
        set(hf,'pointer',cat)
        brick.buttonmotion({@frameresizeborder,hf,hobj,idx,getvalue(info.scale)},hf)
        set(hf,'pointer','arrow')
    case 'extend'
        p0 = getvalue(info.scale)*round(p/getvalue(info.scale));
        set(hf,'pointer','hand')
        brick.buttonmotion({@frameresizemove,hf,hobj,p0,pos,getvalue(info.scale)},hf)
        set(hf,'pointer','arrow')
    case 'alt'
...
setappdata(hf,'brick_framedesign',struct('grob',grob,'fname',fname))
