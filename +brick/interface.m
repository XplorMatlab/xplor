classdef interface < hgsetget
    ...
    %---
    % Interface class provides utilities for designing a graphic brick.interface,
    ...
    % A simple example usage is also provided in [Brick toolbox dir]/examples/example_brick.interface
    %
    % Notes:
    % - to make a new brick.interface, define a new class having brick.interface as a
    %   parent
    % - constructor: 
    %     . in the new object constructor, first call the brick.interface
    %       constructor (X = X@brick.interface(hf,figtitle,defaultoptions)
    ...
    %       brick.interface, it should do it in a init_menus(X), which starts by
    %       calling init_menus@brick.interface(X); menu handles can be stored in
    %       the structure X.menus
    %     . brick.interface overwrites the default set method in order to easily
    ...
    methods
        function I = interface(hf,figtitle,defaultchildoptions)
            ...
            if nargin>=3 && ~isempty(defaultchildoptions)
                defaultoptions = brick.structmerge(defaultoptions,defaultchildoptions); % default set
            end
            ...
            I.options = brick.structmerge(defaultoptions,I.options,'skip');      % update
            saveoptions(I)
            ...
            I.hf = hf;
            I.figtitle = figtitle;
            if hf~=0 % use hf=0 for a command line brick.interface only!
                figure(hf)
            ...
            if strcmp(class(I),'interface'), brick.interface_end(I), end %#ok<STISA>
        end
        ...
        function interface_end(I)
            ...
            % graphic brick.interface or command line only?
            if I.hf~=0
                
                % brick.interface menu
                init_menus(I)
    ...
    methods
        function fname = get.settings(I)
            fname = brick.userconfig('configfolder',[class(I) '.mat']);
        end
        function init_menus(I)
            ...
            uimenu(m,'label','Copy sub-part...', ...
                'callback',@(u,evnt)brick.savefig(I.hf,'showonly','subframe'))
            m1 = uimenu(m,'label','More');
            items.savesub  = uimenu(m1,'label','Use sub-frame','checked',brick.onoff(s.dosavesubframe), ...
            ...
            uimenu(m1,'label','Save image (full options)...', ...
                'callback',@(u,evnt)brick.savefig(I.hf))
        ...
        function saveoptions(I)
            brick.savevar(I.settings,I.options)
        end
        function loaddefaultoptions(I)
            codefile = which(class(I));
            fdefault = [brick.fileparts(codefile,'noext') '.mat'];
            I.options = brick.loadvar(fdefault);
        end
        function savedefaultoptions(I)
            codefile = which(class(I));
            fdefault = [brick.fileparts(codefile,'noext') '.mat'];
            brick.savevar(fdefault,I.options)
        end
        ...
        function chgframepositions(I,flag,varargin)
            ...
            idxsaved = brick.switch_case(iscurtmp,2:npos,1:npos);
            changedlist = false;
            switch flag
                case 'modify'
                    newpos = brick.framedesign(I.grob,curpos,true);
                    if iscurtmp
                        pos(1).value = newpos;
                    else
        ...
        function saveimage(I,fname)
            ...
            if I.interfacepar.dosavesubframe
                ...
                brick.savefig(I.hf,fname{:},I.interfacepar.subframe);
                set(I.interfacepar.hsubframe,'visible','on')
            else
                brick.savefig(I.hf,fname{:})
            end
        end
        function setsaveframe(I,flag)
            switch flag
                case 'toggle'
                    ...
                    set(items.savesub,'checked',brick.onoff(dosubframe))
                    set(I.interfacepar.hsubframe,'visible',brick.onoff(dosubframe))
            ...
            if dodef
                ...
                [I.interfacepar.subframe I.interfacepar.hsubframe] = brick.figselection(I.hf);
                set(I.interfacepar.hsubframe,'visible',brick.onoff(I.interfacepar.dosavesubframe))
            end
        end
        function set(I,f,x)
