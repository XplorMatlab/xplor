...
if nargin==0, help brick.timevector, return, end
...
% Output type
if nargin<3
    outputtype = brick.switch_case(inputtype,'times','count','count','times');
...
if iscell(x)
    ...
    y = cell(1,nx);
    for i=1:nx
        y{i} = brick.timevector(x{i},t{i},outputtype);
    end
    if ~strcmp(outputtype,'times')
        nper = brick.itemlengths(y); % not all counts are necessarily the same time: pad with zeros
        if any(diff(nper))
            npermax = max(nper);
...
convtype = [inputtype(1) '2' brick.switch_case(outputtype,'times','t','c')];
