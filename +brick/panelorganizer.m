classdef panelorganizer < hgsetget
    ...
    %---
    % function O = brick.panelorganizer(hparent,'V|H',n[,dorelative[,xx]])
    ...
    methods
        function O = panelorganizer(hparent,splitmode,varargin)
            ...
            O.szlistener = brick.pixelsizelistener(O.hobj,@(u,e)set(O,'extents',[O.children.extent]));
        end
        function delete(O)
            if ~isvalid(O) && ~isprop(O,'szlistener'), return, end
            brick.delete_valid(O.szlistener)
        end
        function split(O,mode,n,dorelative,xx)
            ...
            if nargin<5
                [W H] = brick.pixelsize(O.hobj);
                x = brick.switch_case(mode,'H',W,'V',H);
                xx = ones(1,n)*(x/n);
            end
            % set
            O.splitmode = mode;
            ...
            [O.children.dorelative] = brick.dealc(dorelative);
    ...
    methods
        ...
        function O1 = setSubOrg(O,idx,splitmode,varargin)
            ...
            c.isset = true;
            c.isorg = true;
            O1 = brick.panelorganizer(c.hobj,splitmode);
    ...
    methods
        function updatePositions(O,idx)
            ...
            for i = idx
                ...
                set(ci.hobj,'visible',brick.onoff(xx(i)>0))
            end
        end
        ...
        function set.extents(O,extents)
            ...
            % update display
            [O.children.extent] = brick.dealc(xx);
            updatePositions(O)
        end
    ...
    methods (Access='private')
        ...
        function moveBorder(O,idx)
            ...
            hf = brick.parentfigure(O.hobj);
            n = O.nchildren;
            if ischar(idx)
                if ~brick.boolean(get(hf,'Resize')), return, end % not allowed to resize figure
                flag = idx;
    ...
    methods (Static)
        function s = test
            s.hf = brick.figure('PANELORGANIZER TEST','handlevisibility','off');
            clf(s.hf)
            s.a = brick.panelorganizer(s.hf,'H',2,[0 1],[300 1]);
...
hf = brick.parentfigure(u);
p0 = get(hf,'currentpoint');
panelpos = brick.pixelpos(u,'recursive');
pixtol = 4;
