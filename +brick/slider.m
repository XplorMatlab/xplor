classdef slider < hgsetget
    %SLIDER     Special control that improves the functionality of Matlab slider
    %---
    % function slider([hp,][properties])
    %---
    % Slider
    %
    ...
    %
    % See also brick.sliderenhance, brick.sensor, brick.control
    ...
    methods
        ...
        function delete(U)
            if ~isempty(U.deletefcn)
                brick.evalcallback(U.deletefcn,U.hpanel,[])
                U.deletefcn = '';
            end
    ...
    methods
        ...
        function set.layout(U,x)
            if ~brick.ismemberstr(x,{'left' 'right' 'up' 'down' 'auto'})
    ...
    methods
        ...
        function width = get.width(U)
            if U.area
                ...
                W = 10;
                fact = U.posframepix(brick.switch_case(isvertical,4,3));
                width = max(W/fact,w);
            end
        end
        ...
        function set.minmax(U,mM)
            ...
            set(U.hslider,'visible',brick.onoff(diff(mM)))
        end
    ...
    methods
        function set.scrollwheel(U,flag)
            switch flag
                case 'on'
                    brick.scrollwheelregister(U.hpanel,@(n)event(U,'scroll',n))
                case 'default'
                    brick.scrollwheelregister(U.hpanel,@(n)event(U,'scroll',n),'default')
                case 'off'
                    brick.scrollwheelregister(U.hpanel,flag)
                otherwise
                ...
                otherwise
                    ...
                    switch flag
                        case 'left'
                            ...
                            pos0 = U.left;
                            [isvert isinv] = orientation(U);
                            set(U.hf,'pointer',brick.switch_case([isvert isinv], ...
    ...
    methods
        function [isvertical iscoordinv] = orientation(U)
            ...
            if strcmp(ori,'auto')
                pos = U.posframepix;
                ori = brick.switch_case(pos(4)>pos(3),'up','right');
            end
            isvertical = brick.ismemberstr(ori,{'up' 'down'});
            iscoordinv = brick.ismemberstr(ori,{'left' 'down'});
        end
...
if strcmp(get(hobj,'type'),'uipanel')
    panelpos = round(brick.pixelpos(hobj,'recursive'));
