function [s grid] = alignimage(a,b,varargin)
%ALIGNIMAGE Manual alignment of 2 images
%---
% function [[s grid] =] alignimage(a,b[,shift[,'nouser']]['slider|edit'])
%---
...
%   shift=brick.alignimage(X,Y)
%
% See also brick.register, brick.translate
...
% Figure
hf = figure(863);
set(hf,'numbertitle','off','name','brick.alignimage')
...
if douser
    ...
    spec = brick.structmerge(spec1,spec2);
    X = brick.control(s,spec,@(x)showimages,'okbutton');
else
    X = s(1);
    X.s = s(1);
end
...
brick.scrollwheelregister(hf,@(n)scrollwheel(n))
ha = axes('parent',hf);
...
if nargout==0 && ~douser
    ...
    function keypress(e)
        ...
        [shft ctrl alt] = brick.flags({'shift' 'control' 'alt'},e.Modifier);
        if shft, d = d*5; end
        if ctrl, d = d/10; end
        if alt, d = d/100; end
    ...
    function showimages(dorecompute)
        % bmov = min(max(brick.translate(b,shift),0),1);
        ...
        im = brick.clip(im,X.clip,X.colormap);
