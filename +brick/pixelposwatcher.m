classdef pixelposwatcher < handle
    ...
    % See also: brick.pixelposlistener, brick.pixelsizelistener, brick.pixelpos, brick.pixelsize
    ...
    methods
        function PP = pixelposwatcher(hobj)
            ...
            if ~PP.isfig && isempty(PP.parent) && ~PP.newgraphics
                ...
                PP.parent = brick.pixelposwatcher(hp);
                brick.connect_listener(PP.parent,PP,'changepos',@(u,e)updatepos(PP));
            end
            PP.pixelpos = zeros(1,4);
            updatepos(PP)
            if PP.newgraphics
                brick.connect_listener(hobj,PP,'LocationChanged',@(hf,e)updatepos(PP));
            else
                brick.connect_listener(hobj,PP,'Position','PostSet',@(hf,e)updatepos(PP));
            end
        end
    end
        ...
        function updatepos(PP)
            ...
            if PP.newgraphics
                ...
                switch get(PP.hobj,'units')
                    ...
                    case {'points' 'inches'}
                        pos = get(PP.hobj,'pos');
                        inchpos = pos / brick.switch_case(get(PP.hobj,'units'),'inches',1,'points',72);
