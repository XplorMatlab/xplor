function imvalue(varargin)
%IMVALUE Automatic link graphs and images for point selection and zooming (try 'brick.imvalue demo')
%---
% brick.imvalue [image] [xy|xonly]
% brick.imvalue clean
% brick.imvalue end
% brick.imvalue('chgx'|'chgy'|'chgxy',newax[,ha])
% brick.imvalue('register'[,ha][,command])
% brick.imvalue('unregister'[,ha])
% brick.imvalue demo
%---
...
if nargin>0 && brick.ismemberstr(varargin{1}, ...
        {'clean','end','chgx','chgy','chgxy','register','unregister','demo'})
    if strcmp(varargin{1},'end'), varargin{1}='terminate'; end
    if ~isappdata(0,'brick_imvalue')
        switch varargin{1}
            case {'clean','end'}
                % nothing to do 
                return
            case {'chgx','chgy','register','unregister'}
                error('brick.imvalue not initialized yet, run ''brick.imvalue'' first')
        end
    end
    feval(varargin{:})
else
    [axisimage xonly xy] = brick.flags('image','xonly','xy',varargin);
...
if isequal(infobase, getappdata(0,'brick_imvalue'))
    % already initialized
    return
end
setappdata(0,'brick_imvalue',infobase)
...
if isappdata(0,'brick_imvalue')
    rmappdata(0,'brick_imvalue')
end
for ha = findobj('type','axes')'
    if strcmp(get(ha,'Tag'),'brick.imvalue')
        terminateAxes(ha)
    end
end
...
if nargin==1, hObject = gca; end
info = getappdata(hObject,'brick_imvalue');
oldax = info.OldAxis;
isimg = info.IsImg;
...
% infobase should already be initialized; info should be only if brick.imvalue
% was already active on this axes
infobase = getappdata(0,'brick_imvalue');
info = getappdata(hObject,'brick_imvalue');
...
% appels automatiques � 'brick.imvalue'
...
% cancel call to brick.imvalue for next lines created in the same axes to
...
% reset, the automatic call to brick.imvalue will be active again
...
if ~strcmp(get(hObject,'Tag'),'brick.imvalue') % le plus probable ; plus rapide de d'abord tester ��
...
if isempty(hlist)
    ...
    info1 = getappdata(hlist(1),'brick_imvalue');
    if isimg
...
% save info
setappdata(hObject,'brick_imvalue',info);
...
info0 = getappdata(0,'brick_imvalue');
if info0.xonly, idx = 1:4; else idx = 1:4; end % ?

if isempty(info), return, end
curaxis = info.CurrentAxis;
ax = axis(hObject);
if length(ax)~=4, rmappdata(hObject,'brick_imvalue'), return, end
...
elseif any(ax~=curaxis)
    %disp('brick.imvalue: external change in axis, error might occur')
...
for ha = findallaxes
    info1 = getappdata(ha,'brick_imvalue');
...
if nargin==0
    ha = [];
    panels = brick.row(allchild(0)); % figures
else
    c = brick.row(allchild(hp));
    ctype = get(c,'type');
...
switch selectiontype
    case 'normal'
        rect = brick.mouse(hObject,'rect-');
        rectpixsize = abs(brick.coordinates(hObject,'a2b',rect(3:4),'vector'));
...
info = getappdata(ha,'brick_imvalue');
if ~info.IsImg
...
setappdata(ha,'brick_imvalue',info)
...
% avoid recursions
evaluating = getappdata(hObject,'brick_imvalue_evalregister');
...
try
    ...
    setappdata(hObject,'brick_imvalue_evalregister',true);
    evalin('base',s.cmd)
    rmappdata(hObject,'brick_imvalue_evalregister');
    if ~isempty(s.hf), set(0,'currentfigure',scf), end
catch %#ok<CTCH>
    try rmappdata(hObject,'brick_imvalue_evalregister'); end %#ok<TRYNC>
