brick.imvalue('function varargout = ')dichotomyfun,varargin)
%DICHOTOMY Uses dichotomy method for one-dimensional optimization
%---
% brick.imvalue('function [x out1 out2 ...] = ')dichotomyfun,lb,ub,tolx,xstart)
% brick.imvalue('function [x out1 out2 ...] = ')dichotomyfun,lb2ub,tolx)
...
while searchtriplet(3)-searchtriplet(1)>tolx && any(diff(e3))
    ...
    if usexstart
        testval = xstart;
        side = brick.switch_case(xstart>searchtriplet(2),'right','left');
        usexstart = false;
    else
        side = brick.switch_case(diff(searchtriplet,2)<0,'left','right');
        testval = mean(searchtriplet(brick.switch_case(side,'left',1:2,'right',2:3)));
    end
