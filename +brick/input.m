...
%INPUT Prompt user for a single value. Function based on structedit
%---
% function x = input([name][,defaultval][,min,max])
% function x = input([name][,defaultval][,spec])
%---
% this is a small wrapper for function structedit, to prompt user for
% a single value; normally if defaultval, min and max are integers, x will
% also be integer
%
% brick.input('text') is ambiguous and will result in a numeric output
% use brick.input('name','text') for a string output (with 'text' as default)
% brick.input('text','char') is also ambiguous and will result in an empty
% default
% use brick.input('name','text','char') to have 'text' as string default
% 
% See also brick.structedit, brick.control, brick.reallydlg
...
if nargin==0
    ...
    elseif nargin==1
        if brick.ismemberstr(brick.regexptokens(a,'^([^ ]*)'),speckeys)
            % this is a specification (no name, no default)
        else
            % this is a name
            name = a;
        ...
        if nargin==2
            ...
            elseif ~ischar(b)
                error 'argument'
            elseif brick.ismemberstr(brick.regexptokens(b,'^([^ ]*)'),speckeys)
                % this is a spec
            else
                % this is a default value
                defaultval = b;
