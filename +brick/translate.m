function [y weight J dweight] = translate(x,shift,varargin)
% function [y weight J dweight] = translate(x,shift[,'full|valid'][,'linear|cubic'][,datatype])
%---
...
%
% See also brick.register, brick.alignimage
...
if size(x,3)>1
    ...
    for i=1:nt, y(:,:,i) = brick.translate(x(:,:,i),shift(:,i),'full',method,datatype); end
...
y0 = x(iin,jin,:);
if strcmp(datatype,'double'), y0 = double(y0); else y0 = brick.float(y0); end
...
p = brick.add( ...
    ...
    -brick.column(shift(1:2)) ...         % inverse translation
    );
