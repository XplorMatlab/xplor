function x = detrend(x,frames,varargin)
%DETREND Remove a linear trend estimated only from specific indices
%---
% function y = detrend(x,frames[,dim][,'subtract|normalize|keepmean'])
%---
...
for k=1:length(varargin)
    ...
    if ischar(a)
        flag = a;
        if ~brick.ismemberstr(flag,{'subtract' 'normalize' 'keepmean'})
...
% perform detrending
x = brick.reshapepermute(x,{1:dim-1 dim dim+1:ndim});
nc = size(x,3);
for k=1:nc
    xk = x(:,:,k);
    xkframes = xk(:,frames);
