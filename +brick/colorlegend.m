function hleg = colorlegend(varargin)
% function [hleg =] colorlegend([handles,]names[,'NorthWest|...'][,'frame'][,text options...])
%---
...
% Parent axes
n = numel(hl);
ha = brick.parentaxes(hl(1));
for i=2:n
    if ~isequal(brick.parentaxes(hl(i)),ha)
...
set(hleg,'visible',brick.onoff(doframe))
[w h] = brick.pixelsize(hleg);
...
brick.controlpositions(hleg,ha,posrel,pospix)
...
brick.propcontrol(hleg,'Visible','menu',{m,'label','Show legend''s frame'});
uimenu(m,'label','Delete legend','callback',@(u,e)delete(hleg))
set([hleg ht],'UIContextMenu',m)

% Delete legend upon deletion of the lines
addlistener(ht(1),'ObjectBeingDestroyed',@(u,e)brick.delete_valid(hleg));
...
brick.moveobject(hleg,'pointer','fleur');

pos = brick.pixelpos(hleg);
center = [pos(1)+pos(3)/2 pos(2)+pos(4)/2];  
posa = brick.pixelpos(ha);
