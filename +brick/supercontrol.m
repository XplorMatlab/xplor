classdef supercontrol < hgsetget
    %SUPERCONTROL   Super arrangement of control
    %---
    % function X = supercontrol([hp,]specs[,callback[,x0]])
    % function specs = supercontrol.examplespecs
    % function supercontrol('demo')
    %---
    ...
    %
    % Output:
    % - X           brick.supercontrol object; X.x is a structure that
    ...
    % Constructor
    methods
        function X = supercontrol(varargin)
            % function X = supercontrol([hp,]specs[,callback[,x0]])
            % input
            ...
            elseif ischar(varargin{1}) && strcmp(varargin{1},'demo')
                hp = figure;
                varargin = {brick.supercontrol.examplespecs,@disp};
            else
            ...
            namelengths = brick.map(@length,{specs.name});
            ...
            colprop = brick.switch_case(get(hp,'type'),'figure','color','uipanel','backgroundcolor');
    ...
    methods
        ...
        function b = get.immediateupdate(X)
            b = brick.boolean(get(X.immupdcontrol,'value'));
        end
    ...
    methods
        ...
        function display_init(X)
            ...
            X.slider = brick.slider('parent',X.hp, ...
        ...
        function display_linepositions(X,klines)
            ...
            for i=klines
                ...
                set(X.controls(i).panel,'position',posi, ...
                    'visible',brick.switch_case(posi(2)>0 && posi(2)+posi(4)<=H))
            end
        end
            ...
            for k=1:ncontrol
                ...
                switch speck.style
                    ...
                    case {'file' 'dir'}
                        ...
                        if isfield(speck,'string'), set(content{k},'string',speck.string); end
                    case 'stepper'
                        content{k} = brick.stepper('parent',contentpanel);
                    otherwise
        ...
        function chgfilevalue(X,u,k,mode)
            fname = brick.getfile(brick.switch_case(mode,'file','SAVE','dir','DIR'));
            set(u,'string',fname)
            data_userset(X,u,k)
        end
        function display_contentvalue(X,i)
            ...
            for k=1:ncontrol
                ...
                if length(X.x(i).value)<k
                    disp 'problem in brick.supercontrol'
                    val = spec(k).default;
                else
                    val = X.x(i).value{k};
                end
    ...
    methods
        function data_userset(X,u,k)
            ...
            switch speck.style
                ...
                case {'edit' 'file' 'dir'}
                    ...
                    elseif ~isfield(speck,'type')
                        ...
                        [val errormsg] = brick.chardisplay(str,speck.type);
                        if ~isempty(errormsg)
            ...
            % eval brick.supercontrol callback
            X.activechg = X.x(i).active;
            evalfun(X)
        end
        function data_toggleactive(X,u)
            i = get(get(u,'parent'),'userdata');
            X.x_data(i).active = brick.boolean(get(u,'value'));
            % eval brick.supercontrol callback
            X.activechg = true;
            evalfun(X)
        end
        function data_addline(X)
            k = brick.switch_case(isscalar(X.specs),1,get(X.addlinecontrol,'value')-1);
