function pcorr = pcorrect(pvalues,method,varargin)
%PCORRECT Perform correction for multiple testing
%---
% function pcorr = pcorrect(pvalues,method[,dim])
% function test = pcorrect(pvalues,method,alpha[,dim])
%---
...
if ~isvector(pvalues) 
    ...
    pvalues = brick.reshapepermute(pvalues,{dim dimc});
    pcorr = pvalues;
    if isempty(alpha), alpha = {}; else alpha = {alpha}; end
    for i=1:size(pvalues,2)
        pcorr(:,i) = brick.pcorrect(pvalues(:,i),method,alpha{:});
    end
    perm = zeros(1,nd); perm(dim) = 1:length(dim); perm(dimc) = length(dim)+1:nd;
    pcorr = brick.reshapepermute(pcorr,[s(dim) s(dimc)],perm);
    return
end
...
if any(inan)
    pcorr = pvalues;
    if isempty(alpha), alpha = {}; else alpha = {alpha}; end
    pcorr(~inan) = brick.pcorrect(pvalues(~inan),method,alpha{:});
    return
end
...
switch lower(method)
    ...
    case {'benjamini-hochberg','fdr','bh'}
        [~, ord] = sort(pvalues); ord = brick.row(ord);
        pcorr = pvalues; pcorr(ord) = brick.row(pvalues(ord)).*(n./(1:n));
        p=0; for i=ord, [p pcorr(i)] = deal(max(p,pcorr(i))); end 
    otherwise
