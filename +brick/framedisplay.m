function out = framedisplay(varargin)
%FRAMEDISPLAY Display images arranged as a grid
%---
% [im/ha =] function framedisplay([x,[y,...]]data[,clip][,'singleaxes|multaxes'] ...
...
if nargin==0, help brick.framedisplay, return, end
% Input
% (scan varargin)
data = []; rangearg = {}; 
clip = []; axesflag = ''; in = []; 
...
if founddata<=0
    if ~ismember(length(rangearg),[1 3])
        error 'input to brick.framedisplay.m is ambiguous, use ''clip'', ''in'', ''ncol'' flagws'
    end
    data = rangearg{end};
    rangearg(end) = [];
end
...
if isempty(axesflag)
    ...
    elseif length(rangearg)>2 || ...
        ...
        axesflag = brick.switch_case(prod(nt)>6,'singleaxes','multaxes');
    end
end
% (x,y)
...
if nc==1
    ...
    clip = brick.clip(data,clip,'getrange');
...
switch axesflag
    ...
    case 'multaxes'
        dodisplay = true;
        if isempty(in) || (isscalar(in) && brick.ismemberstr(get(in,'type'),{'figure' 'uipanel'}))
...
if dodisplay
    if strcmp(axesflag,'multaxes')
        siz = brick.pixelsize(hf);
    else
        siz = brick.pixelsize(ha(1));
    end
    haratio = siz(2)/siz(1);
else
    haratio = 1;
...
% one or several axes?
if brick.ismemberstr(axesflag,{'singleaxes' 'output'})
    ...
    elseif dodisplay
        ...
        if ~isempty(t1) && (~isnumeric(t1) || any(diff(diff(t1))))
            if isnumeric(t1), t1 = brick.num2str(t1,'cell'); end
            set(ha,'xtick',1:ncol,'xticklabel',t1)
        end
        if ~isempty(t2) && (~isnumeric(t2) || any(diff(diff(t2))))
            if isnumeric(t2), t2 = brick.num2str(t2,'cell'); end
    ...
    % frame ranges
    if ~iscell(t1), t1 = brick.num2str(t1,'cell'); end
    if ~iscell(t2), t2 = brick.num2str(t2,'cell'); end
    % display loop
    k = 0;
    for j=1:nrow
        for i=1:ncol
