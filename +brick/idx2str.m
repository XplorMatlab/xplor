function str = idx2str(val,delimiters)
%IDX2STR Convert indices to a compact string representation, e.g. '1:2 5:8' for [1 2 5 6 7 8]
%---
% function str = idx2str(val[,delimiters])
%---
...
%
% Example:
% brick.idx2str([1 2 3 4 6 8 10 15]) returns '1:4 6:2:10 15'
...
elseif any(d==1)
    ...
    if k>1, str = [brick.idx2str(val(1:k-1)) sep]; else str = ''; end
    str = [str num2str(val(k)) cont num2str(val(k+n))];
    if k+n<length(val), str = [str sep brick.idx2str(val(k+n+1:end))]; end
