function [y freqs yticklog yticklabel] = spectrogram(x,dt,varargin)
%SPECTROGRAM Compute spectrogram
%---
% function [y freqs yticklog yticklabel] = spectrogram(x,dt[,freqscaling][,freqs])
% function spectrogram(['display',]y,dt|tt,freqs,yticklog,yticklabel[,freqscaling][,clip])
%---
...
if nargin==0, help brick.spectrogram, end
...
y = zeros([nt nscale s]);
if nc>1, brick.progress('spectrogram',nc), end
for kc=1:nc
    xk = x{kc};
    nx = size(xk,2);
    if nc>1, brick.progress(kc), elseif nx>1, brick.progress('spectrogram',nx), end
    % remove baseline and pad with zeros on the sides
    npad = floor(nt/2);
    xk = [zeros([npad nx]); brick.normalize(xk,1,'-'); zeros([npad nx])];
    % compute and average
    for i=1:nx
        if nc==1 && nx>1, brick.progress(i), end
...
yticklabel = brick.num2str(ytick,'cell');

% frequency scaling
if ~isempty(freqscaling)
    freqscales = arrayfun(freqscaling,freqs);
    y = brick.mult(y,freqscales);
end
...
if nc==1
    ...
    ha = brick.framedisplay(t,log10(freqs),y,'multaxes','normalaxis',clip{:});
