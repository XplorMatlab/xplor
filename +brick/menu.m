...
%MENU Utility to create a basic GUI made of a line of buttons
%---
% function m = menu([fig,]'h'|'v',title,width,height,'FramePropertyName',FramePropertyValue,...)
% function u = menu(m,'UIControlPropertyName',UIControlPropertyValue,...)
% function [m u] = menu([menu options],'add',[first control options],'add',[second control options],...)
%---
...
docreate = brick.ismemberstr(get(h,'type'),{'figure' 'uipanel'});
if docreate
    hf = h;
else
    m = h;
...
if info.doresizefig, brick.setfigsize(hf,position(3:4)+10), end
