function repout = cd(flag,varargin)
%CD User definition of shortcut to fast access directories
%---
% function rep = cd(flag,relpath)
% function cd('edit')
% function [list = ] cd('list')
%---
% This function allows user to rapidly change the current directory or get
% the full path to some files. 
%
% - Type 'brick.cd edit' to launch a GUI brick.interface that enables you to define
%   your own absolute and relative paths (for example the full path to your
%   home, to which will be associated the flag 'home')
% - Then type 'brick.cd home' to set Matlab current directory to your home.
% - Type 'a = brick.cd('home');' to get the full path to your home (note that
%   this will not change Matlab current directory
% - Additional arguments to brick.cd can be used to access subdirectories or
%   files inside a flagged folder. For example, type 'brick.cd home dir1 dir2'
%   to set Matlab current directory to the subdirectory dir1/dir2 inside
%   your home directory, or type 'a = brick.cd('home','dir1/myfile');' to get
...
if nargin==0, help brick.cd, return, end
...
switch flag
    ...
    case 'list'
        ...
        if nargout==1
            ...
            list = brick.reshapepermute(list,[nrow ncol nc],{1 [3 2]});
...
if isstruct(path)
    khost = strcmp(brick.hostname(),{path.host});
...
set(hf,'numbertitle','off','name',['CD (current host: ' brick.hostname() ')'])
...
% sort definitions
[~, ord] = sortrows([brick.map(s,@(u)char('A'+~isstruct(u.path)),'cell')' {s.relto}' {s.label}']);
s = s(ord);
...
displaypage()
    ...
    function displaypage()
        for k=1:ndefperpage
            ...
            if kdef>ndef || isempty(s(kdef).label)
                ...
                % definition exists
                sk = s(kdef);
                brick.set(hu(:,k),'string',{sk.label,sk.relto,resolvehost(sk.path)})
    ...
    function chgdef(k)
        ...
        if get(hm(k),'value')
            ...
            sk = s(kdef);
            path = struct('host',brick.hostname(),'path',path);
            if isstruct(sk.path)
