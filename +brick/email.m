function m = email(flag)
%EMAIL Send e-mails from Matlab! Automatically attach figures, M-files and more
%---
% function email
% function email('setserver')
%--------------
% sends an email !!
...
while str
    if isempty(strfind(str,'@'))
        emails = brick.userconfig('brick.email');
        ...
        if ~isfield(emails,str)
            ...
            brick.userconfig('brick.email',emails)
            str = strfull;
        else
...
elseif brick.ismemberstr(str,{'cur' 'gcf'})
    f = gcf;
else
...
for i=1:length(f)
    ...
    figname{i} = brick.autofigname(f(i)); %#ok<*AGROW>
    for j=1:length(fformat)
        brick.savefig(f(i),[figname{i} '.' fformat{j}],fformat{j})
...
MESSAGE = [MESSAGE '' '[mail sent from Matlab using brick.email, see http://trac.int.univ-amu.fr/brick]'];
...
if ~any(findstr(str,'n'))
    ...
    try 
        ...
        disp 'failed!'
        disp(ME.message)
        disp 'Try ''brick.email setserver'' to set your Server parameters,'
