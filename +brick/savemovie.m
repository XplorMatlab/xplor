function savemovie(a,varargin)
% function savemovie(a[,fname][,clip][,fps][,zoom][,map]
...
% - clip    a 2-values vector, or clip flag (see brick.clip) [by default, no
...
% the value by a flag.
% e.g. brick.savemovie(rand(3,4,25),'fname','test.avi','zoom',10)
%
% See also brick.readmovie, brick.movie, VideoWriter

% Thomas Deneux
% Copyright 2004-2017

if nargin<1, help brick.savemovie, return, end
...
if nargout==0 && isempty(fname)
    fname = brick.savefile('*.avi','Select file to save movie.');
...
if ~isempty(clip)
    ...
    a = brick.clip(a,clip); % double values between 0 and 1
end

% check compression
ext = lower(brick.fileparts(fname,'ext'));
