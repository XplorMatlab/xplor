...
%REGISTER Coregister images or movie
%---
% function [shift e xreg] = register(x,par|ref)
% function par = register('par')
%---
% Be careful with the sign: xreg is obtained as
% xreg = brick.translate(x,-shift);
...
%
% See also brick.translate, brick.xregister, brick.alignimage

% Thomas Deneux
% Copyright 2011-2017

if nargin==0, help brick.register, end
...
if ischar(x)
    ...
    elseif isstruct(varargin{2})
        if nargin>=2, par = brick.structmerge(par,varargin{2},'strict'); end
    else
        par.ref = varargin{2};
    end
...
for i = 1:ncol
    refi = ref(:,:,i);
    ref(:,:,i) = (refi-brick.nmean(refi(:)))/brick.nstd(refi(:)); % normalize image
end
% filtering
...
par.maxshift = brick.column(par.maxshift);

% Display
if brick.ismemberstr(par.display,{'iter' 'final'})
    hf = brick.figure('brick.register');
    ha = axes('parent',hf);
...
% Register
opt = optimset('Algorithm','active-set','GradObj',brick.onoff(par.dogradobj), ...
    'tolx',par.tolx,'tolfun',par.tolfun,'maxfunevals',1000, ...
    'display',brick.switch_case(par.display,'framenumber','none',par.display));
Q = brick.column(par.FACT);
shift = zeros(2+par.dorotation+par.doscale,nt);
if nt>1 && ~strcmp(par.display,'none'), brick.progress('register frame',nt), end
...
for k=1:nt
    if nt>1 && ~strcmp(par.display,'none')
        brick.progress(k)
        if brick.ismemberstr(par.display,{'iter' 'final'}), set(htitl,'string',sprintf('Registration %i/%i',k,nt)), end
    end
    xk = double(x(:,:,k,:));
    if docut, xk = xk(oki,okj,:,:); end
    for i = 1:ncol
        xki = xk(:,:,i);
        xk(:,:,i) = (xki-brick.nmean(xki(:)))/brick.nstd(xki(:)); % normalize image
    end
    % reset current shift?
    if k==1 || ~par.useprev0
    ...
    if e0==0
        ...
        % global registration
        if par.doxreg
            d = brick.xregister(xk,ref,xregminoverlap);
        end
...
if par.tsmoothing
    ...
    shift = brick.filt(shift,par.tsmoothing,2);
    shift = shift(:,nt+1:2*nt);
end
if nargout<3, return, end

% Resample
if nt>1 && ~strcmp(par.display,'none'), brick.progress('resample frames'), end
xreg = zeros(ni,nj,nt,class(x)); %#ok<*ZEROLIKE>
for k=1:nt
    for i=1:ncol
        xreg(:,:,k,i) = brick.translate(x(:,:,k,i),-shift(:,k),'full');
    end
end
...
if par.dorotation || par.doscale
    ...
    p = brick.add( ...
        M*[brick.row(ii); brick.row(jj)], ...   % inverse rotation with center (0,0)
        center-M*center ...         % translation to have a rotation with center the center of the image
        -brick.column(d(1:2)) ...         % inverse translation
        );
    if isempty(par.mask)
    ...
    [xpred weight] = brick.translate(ref,d,'valid');
end
% end
mask = logical(weight);
