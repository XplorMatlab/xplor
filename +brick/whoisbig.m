...
for k=1:length(varargin)
    ...
    elseif ischar(a)
        ...
        if ~isempty(tokens)
            [n u] = deal(tokens{1}{:});
            minsize = brick.switch_case(isempty(n),1,str2double(n)) * 2^brick.switch_case(lower(u),'',0,'k',10,'m',20,'g',30);
        else
            try
                varname = a;
...
%% size
s1 = fliplr(char(brick.map(@(x)fliplr(num2str(x(1))),{w.size},'cell')));
s2 = char(brick.map(@(s)brick.strcat(s(2:end),'x','x',''),{w.size},'cell'));
