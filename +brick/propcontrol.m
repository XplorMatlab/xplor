classdef propcontrol < hgsetget
%PROPCONTROL Create controls automatically linked to an object property
%---
% function propcontrol(obj,prop,spec,graphic object options...)
% function propcontrol(obj,prop,spec,{graphic object options...})
% function ctrl = propcontrol.createcontrol(...)
%---
% Create a control that will be synchronized to an object property value.
% Use the static method brick.propcontrol.createcontrol (3rd syntax above) to
% return the graphic handle of the control rather than the brick.propcontrol
% object.
% 
% Input:
...
%           array.
% Output:
% - ctrl    object of class brick.propcontrol
%           the graphical object that has been created can be found in its
%           property ctrl.hu
%
% See also: brick.menugroup, brick.control, brick.setpropertyandmark
...
methods
    function M = propcontrol(obj,prop,spec,varargin)
        ...
        switch spec
            ...
            case {'menuval' 'menugroup'}
                M.type = 'list';
                M.style = 'menugroup';
                menustyle = brick.switch_case(spec,'menuval','sub+val','menugroup','group');
            case 'menu'
    ...
    function delete(M)
        ...
        brick.delete_valid(M.proplistener,M.hu,M.hparent)
    end
end
...
methods
    function updatevalue(M)
        ...
        if M.docolor
            ...
            [colornum, colorname] = brick.colorbyname(curval);
            if ~isempty(colornum), curval = colornum; end
        end
        switch M.style
            % type logical or on/off
            case 'menu'
                M.hu.Checked = brick.onoff(curval);
            case {'checkbox' 'radiobutton' 'togglebutton'}
            ...
            case 'edit'
                set(M.hu,'string',brick.chardisplay(curval))
            % list of values
            case 'menugroup'
                set(M.hu,'checked','off')
                if M.docolor
                    idx = brick.find(curval,M.valuelist); % color values are stored as numerical values
                else
        ...
        switch M.style
            ...
            case 'menu'
                if strcmp(M.type,'on/off')
                    set(M.obj,M.prop,brick.onoff(~brick.boolean(get(M.hu,'checked'))));
                else
                    set(M.obj,M.prop,~brick.boolean(get(M.hu,'checked')));
                end
            case {'checkbox' 'radiobutton' 'togglebutton'}
                if strcmp(M.type,'on/off')
                    set(M.obj,M.prop,brick.switch_case(get(M.hu,'checked'),'on/off'));
                else
                    set(M.obj,M.prop,brick.boolean(get(M.hu,'value')));
                end
            % edit
            case 'edit'
                if strcmp(M.type,'char')
                    ...
                    set(M.obj,M.prop,brick.chardisplay(get(M.hu,'value'),M.type));
                end
            % list of values
            case 'menugroup'
                ...
                elseif i>length(M.valuelist) && M.doother
                    value = brick.input(['enter ' M.prop ' value:'],get(M.obj,M.prop));
...
methods
    function setValueList(M,valuelist,labellist,shortlabels)
        ...
        % special: color
        M.docolor = false;
        if ~isempty(regexpi(M.prop,'color')) && any(brick.map(@ischar,M.valuelist))
            % try converting color names to colors
            try
                [colornum colorname] = deal(cell(1,length(M.valuelist)));
                for i=1:length(colornum)
                    [colornum{i} colorname{i}] = brick.colorbyname(M.valuelist{i},'strict');
                end
        ...
        if ~isempty(M.valuelist)
            if isempty(M.labellist), M.labellist = brick.num2str(M.valuelist); end
...
methods
    function set.visible(M,b)
        M.visible = brick.boolean(b);
        set([M.hparent M.hu],'visible',brick.onoff(b)) %#ok<*MCSUP>
    end
    function set.enabled(M,b)
        M.enabled = brick.boolean(b);
        set([M.hparent M.hu],'enabled',brick.onoff(b)) %#ok<*MCSUP>
    end
end
