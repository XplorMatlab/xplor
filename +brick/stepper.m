classdef stepper < hgsetget
    %STEPPER    Edit a numeric value, includes increment/decrement buttons
    %---
    % function stepper([n,][properties])
    % See also brick.slider, brick.control
    ...
    methods
        function P = stepper(varargin)
            ...
            P.hpanel = uipanel('parent',P.parent, ...
                ...
                'bordertype','none');
            brick.pixelsizelistener(P.hpanel,@(u,e)updateinternposition(P));
    ...
    methods
        ...
        function updateinternposition(P)
            pixsiz = brick.pixelsize(P.hpanel);
            w = pixsiz(1)/P.nx;
            h = pixsiz(2);
            if h>=12
    ...
    methods
        function set.value(P,val)
            if P.doset
                ...
                if P.coerce && P.step, val = brick.round(val,P.step); end
                for k=1:P.nx
...
if isempty(fmt)
    fmt = brick.switch_case(mod(x,1),'%.2g','%i');
end
str = num2str(x,fmt);
