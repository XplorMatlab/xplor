function [y weight] = affinity(x,M,varargin)
%AFFINITY   Perform affine transformation on an image
%---
% function [y weight] = affinity(x,M[,'linear|cubic'][,datatype])
%---
...
if size(x,3)>1
    ...
    for i=1:nt, y(:,:,i) = brick.translate(x(:,:,i),M(:,:,i),'full',method,datatype); end
...
q = M1 * [ones(1,ni*nj); brick.row(ii); brick.row(jj)];
qi = floor(q);
