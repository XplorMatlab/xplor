function im = printnumber(im,elems,varargin)
%PRINTNUMBER Add small numbers and text to images or movies
%---
% function im = printnumber(im,num|str|scale,options...)
%---
...
if nargin==0, help brick.printnumber, return, end
...
if isempty(pos)
    pos = brick.switch_case('doscale','bottomleft','topleft');
end
...
switch fontsize
    case '3x5'
        ...
        patterns = brick.num2str(permute(patterns,[1 3 2]),'%.3i0'); % nrow x ncol x npattern
    ...
    case {'8x12' '8x16'}
        ...
        try
            ...
            a = brick.readimg(fullfile(brick.fileparts(which('xplor'),'path'),['ascii' fontsize '.png']));
        end
        a = logical(a);
        switch fontsize
            case '8x12'
                ...
                a = brick.reshapepermute(a,[8+8 16 12 16],[1 3 2 4],[8+8 12 256]);
...
N = length(str);
if isempty(im)
    s = [ncol*max(brick.itemlengths(str))+1 nrow+1 N];
