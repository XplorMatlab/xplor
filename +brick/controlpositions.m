function controlpositions(hu,hp,posrel,pospix)
%CONTROLPOSITIONS Set an object position using a combination of absolute and relative (to any other object) coordinates
%---
% function controlpositions(hu,hp,posrel,pospix)
% ---
...
if nargin==0, help brick.controlpositions, return, end

% input
posrel = brick.row(posrel); posrel(end+1:4)=0;
if nargin<4, pospix = []; end
pospix = brick.row(pospix); pospix(end+1:4)=0;

% delete previous listeners
if isgraphics(hu)
    hl = getappdata(hu,'brick_controlpositions');
    if ~isempty(hl), deleteposlisteners(hl), end
end

% pointer to listeners
hl = brick.pointer('ppos',[],'axlim',[],'axratio',[]);
...
% set listeners
if hp==get(hu,'parent')
    hl.ppos = brick.pixelsizelistener(hp,updatefcn);
elseif get(hp,'parent')==get(hu,'parent')
    hl.ppos = brick.pixelposlistener(hp,updatefcn);
...
if isgraphics(hu)
    setappdata(hu,'brick_controlpositions',hl)
end
...
brick.delete_valid(hl.ppos,hl.axlim,hl.axratio)
...
if hp==get(hu,'parent')
    pos0 = [0 0];
    psiz = brick.pixelsize(hp);
elseif get(hp,'parent')==get(hu,'parent')
    ppos = brick.pixelpos(hp,'strict');
    pos0 = ppos(1:2);
    psiz = ppos(3:4);
else
    if brick.dodebug
