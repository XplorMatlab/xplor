function idx = find(fun,A,varargin)
%FIND Look for non-empty elements, or which evaluate to true when applying a certain function
%---
% function idx = find(fun,A[,'first|last|all|any'][,'columns|rows'])
% function idx = find(A,fun[,'first|last|all|any'][,'columns|rows'])
% function idx = find(x,A[,'first|last|all|any'][,'columns|rows'])
% function idx = find(A[,'first|last|all|any'][,'columns|rows'])
%---
...
% fun = @(y)isequal(y,x) is used, i.e. brick.find looks for occurences of x
% inside A
%
% if only A is provided, function fun = @(y)~isempty(y) is used, i.e.
% brick.find looks for non-empty elements in A, which typically is a cell
% array
%
% See also brick.map, brick.isemptyc

% Thomas Deneux
% Copyright 2016-2017

if nargin==0, help brick.find, return, end
