classdef windowcallbackmanager < hgsetget
    ...
    % brick.scrollwheelregister, but is not supposed to be used by users
    %
    % See also brick.scrollwheelregister, brick.buttonmotion
    ...
    methods
        function W = windowcallbackmanager(hf)
            ...
            brick.pixelposlistener(hf,@(u,e)updatepos(W,'chgfigpos'))
        end
    end
    ...
    methods
        function register(W,hobj,callback,doscroll)
            ...
            if isempty(kobj)
                ...
                W.objects(kobj).doscroll = doscroll && brick.boolean(hobj.Visible);
                % listeners
                brick.pixelposlistener(hobj,@(h,e)updatepos(W,'chgpos',hobj))
                brick.connect_listener(hobj,W,'Visible','PostSet',@(h,e)setactive(W,hobj,brick.boolean(hobj.Visible)));
        ...
        function setactive(W,hobj,value)
            ...
            updatepos(W,brick.switch_case(value,'add','remove'),hobj)
        end
    ...
    methods
        ...
        function updatepos(W,flag,kobj)
            ...
            % some variables
            siz = round(brick.pixelsize(W.hf));
            ...
            switch flag
                case 'addnew'
                    ...
                    if ~isempty(W.scrollmap_internal)
                        pos = round(brick.pixelpos(hobj,'recursive'));
                ...
                case 'set'
                    ...
                    for kobj=2:nobj % start at 2, first element is the figure!
                        ...
                        if sk.doscroll && ishandle(hobj)
                            pos = round(brick.pixelpos(hobj,'recursive'));
                            pos = brick.minmax('axi',[pos(1) pos(1)+pos(3) pos(2) pos(2)+pos(4)],[1 siz(1) 1 siz(2)]);
    ...
    methods
        function keyboardmonitor(W,e,dir)
            if ~brick.ismemberstr(e.Key,{'shift' 'control'}), return, end
            switch dir
                case 'press'
