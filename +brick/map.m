function B = map(fun,A,varargin)
%MAP Apply a given function to the elements, columns or rows of an array (more versatile than Matlab arrayfun)
%---
% function B = map(fun,A[,'columns|rows'][,'array|cell|arraydirect'][,errorval])
% function B = map(A,fun,...)
%---
...
%
% See also brick.isemptyc, brick.itemlengths, brick.find

% Thomas Deneux
% Copyright 2006-2017

if nargin==0, help brick.map, return, end
...
for i=1+dooutput:n
    if manageerror
        if ~dooutput, error 'brick.map cannot manage errors if no output', end
        try
            b = fun(A{i});
