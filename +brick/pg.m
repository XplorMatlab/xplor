...
% function pg([prompt,]'i'[,'max'])
%---
% this is a shortcut for using brick.progress: instead of initializing before
% a loop with brick.progress(prompt,max), and then updating at each loop with
% brick.progress(i), only write pg(promt,i,max) inside the loop (the proper
