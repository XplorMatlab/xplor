function indices = listedit(varargin)
% function indices = listedit(A[,B...][,indices])
%---
...
P.nperlist = brick.map(@length,P.lists);
P.columns = brick.map(@(i)[(1:P.nperlist(i))' ones(P.nperlist(i),1)],1:P.nlist,'cell');
columns2table(P)
...
fposname = [brick.fileparts(which('brick.listedit'),'noext') '.mat'];
brick.framedesign(g,fposname);
...
j = 0;
icol = ones(1,P.nlist);
npercol = brick.map(@(x)size(x,1),P.columns);
...
ii = brick.row(unique(P.cursel(:,2)-1)); % which list(s)
