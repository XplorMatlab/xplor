function c = watch(hf,varargin)
%WATCH Change the pointer to a watch during long computations
%---
% function c = watch([hf])
%---
...
% disp 'brick.watch has a bug, skip it'
% c = [];
% return
...
if nargout==0
    ...
    % stopping
    if brick.dodebug, warning 'syntax for brick.watch function has changed!', end
...
% times
t = getappdata(hf,'brick_watch_timer');
if isempty(t)
    t = timer('StartDelay',.5,'TimerFcn',@(u,e)startfcn(hf));
    setappdata(hf,'brick_watch_timer',t)
