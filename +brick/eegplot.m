function hl = eegplot(varargin)
% function hl = eegplot([t,]data[,stepflag][,flag][,line properties...])
%---
...
if ischar(ystep)
    ...
    switch lower(tokens{2})
        case 'std'
            ystep = fact*brick.nmean(brick.nmean(brick.nstd(data)));
        case 'fit'
...
% dispatch data
if donum
    data = 1+(-1)^strcmp(donum,'top')*brick.normalize(data,1,'-')/ystep;
    ystep = 1;
end
...
if ~isempty(hl)
    ...
    ncol = size(cols,1);
    for k=1:nstep
        set(hl(:,k),'color',cols(brick.mod(k,ncol),:),lineopt{:}) % important to have lineopt here, in case it overwrites the color
    end
    axis(ha,'tight')
