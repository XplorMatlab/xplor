...
%  P = brick.interactivePolygon('spline','closed')    % create the object
...
% Copyright 2015-2017
    ...
    methods
        function P = interactivePolygon(varargin)
            ...
            elseif strcmp(P.points,'init')
                ax = axis(P.ha);
                P.points = brick.add(ax([1 3])',brick.mult([diff(ax(1:2)); diff(ax(3:4))],[.3 .7 .5; .3 .4 .7]));
            end
        ...
        function editPoints(P,flag)
            ...
            switch flag
                case 'move/remove'
                    ...
                    switch flag
                        case 'move'
                            brick.buttonmotion(@()updateLine(P,'idx',idx),'pointer','hand')
                        case 'remove'
                            P.points(:,idx) = [];
                            if isempty(P.points)
                                delete([P.hl P.hl2])
                ...
                case 'add'
                    ...
                    brick.buttonmotion(@()updateLine(P,'idx',idx+1),'pointer','hand')
            end
        end
        ...
        function interpolate(P)
            ...
            if strcmp(P.interpmode,'spline') && P.closed
                pp = pp(:,brick.mod([n-1:n 1:n 1:3],n));
                xx = 3:dx:size(pp,2)-2;
            else
                xx = 1:dx:size(pp,2);
            end
    ...
    methods (Static)
        function poly = drawpoly(varargin)
            P = brick.interactivePolygon('noedit',varargin{:});
            poly = P.points;
        end
    end
