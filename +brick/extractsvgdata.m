classdef extractsvgdata < handle
    %EXTRACTSVGDATA Extract data from SVG file! (PDFs can be converted to SVG with InkScape free software)
    %---
    % function extractsvgdata(fname)
    %---
    ...
    % Read and display
    methods
        function E = extractsvgdata(fname)
            
            % Input
            if nargin==0
                fname = brick.getfile('*.svg','Select .svg file to read');
                %fname = evalin('base','x');
            end
            if ischar(fname)
                xml = brick.readxml(fname);
            ...
            [warncurve] = brick.dealc(warnings);
            if warncurve
                disp('B�zier commands and elliptical arcs will not be displayed properly')
            end
        ...
        function warnings = readoneelem(E,g)
            ...
            if isfield(g.ATTRIBUTE,'transform')
                ...
                switch type
                    case 'translate'
                        translation = brick.column(str2num(tokens{2}));
                        rotation = 1;
                    case 'scale'
                        translation = [0;0];
                        rotation = brick.column(str2num(tokens{2}));
                    case 'matrix'
                        spec = str2num(tokens{2}); %#ok<ST2NM>
                        if length(spec)~=6, error 'could not read matrix data', end
                        translation = brick.column(spec(5:6));
                ...
                for k=1:length(parts)
                    ...
                    switch command
                        case {'m' 'M'}
                            ...
                            if k>1
                                p0 = poly(:,end);
                                polys{end+1} = brick.add(translation,rotation*poly); %#ok<AGROW>
                                poly = [];
                            else
                                p0 = 0;
                            end
                            if strcmp(command,'m')
                                % relative moveto
                                points = brick.add(p0,cumsum(points,2));
                            end
                        case 'L'
                            % absolute lineto: nothing to do
                        case 'l'
                            % relative lineto
                            points = brick.add(poly(:,end),cumsum(points,2));
                        case {'C' 'S' 'Q' 'T' 'A'}
                            % absolute B�zier/arc
                            warncurve = true;
    ...
    methods
        ...
        function setaxis(E,flags)
            ...
            dataval = brick.structedit(dataval);
        ...
        function zoomin(E)
            ax = brick.mouse(E.ha,'rectax-');
            if ~diff(ax(1:2)) || ~diff(ax(3:4)), return, end
            axis(E.ha,ax)
        end
        function zoomout(E)
            E.curaxis = [];
            brick.axis(gca,'tight',1.2)
