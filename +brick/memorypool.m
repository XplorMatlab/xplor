classdef memorypool < handle
    ...
    methods (Static)
        ...
        function item = item(varargin)
            pool = memorypool;
            item = brick.memorypoolitem(pool,varargin{:});
        end
        function setmaxmem(mem)
            ...
            if ischar(mem)
                ...
                unitlog = brick.switch_case(lower(token{2}),'',0,'k',10,'m',20,'g',30','t',40);
        ...
        function clear()
            if brick.dodebug, disp('explicit kill all memorypool items'), end
            pool = memorypool;
            kill(pool.items)
            pool.items(:) = [];
        end
    ...
    methods
        ...
        function checkmemory(pool,safeitem)
            ...
            if any(badidx)
                ...
                %if brick.dodebug, fprintf('auto kill (%s)\n',pool.items(badidx).description), end
