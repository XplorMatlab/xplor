function p = bootstrap(a,b,varargin)
%BOOTSTRAP Compare the mean or median of two ensembles using bootstrap method
%---
% function p = bootstrap(a,b[,mode][,'tail','left|right|both'][,'nperm',npermmax])
%---
...
% brick.progress(['computing ' num2str(nx) ' p-values, permutation'],curcheck)
fprintf('computing %i p-values with %i permutations\n',length(idxx),curcheck)
nperm = 0;
while nperm<checkups(end)
    nperm = nperm+1;
    %if ~mod(nperm,curstep), brick.progress(nperm), end
    if docompare
    ...
    if any(nperm==checkups)
        ...
        if nperm<checkups(end)
            ...
            %             curstep = curcheck/100;
            %             brick.progress(['computing ' num2str(length(idxx)) ' p-values, permutation'],curcheck)
            fprintf('computing %i p-values with %i permutations\n',length(idxx),curcheck)
        end
    end
end
