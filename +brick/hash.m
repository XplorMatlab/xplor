function h = hash(inp,varargin)
%HASH Unique hash number for an array/cell/structure (Copyright M Kleder)
%---
% function h = hash(input[,meth][,'char|hexa|HEXA|num'][,n])
%---
...
if nargin==0, help brick.hash, return, end

meth = 'MD2'; n = 0; outtype = 'char';
for i=1:length(varargin)
    a = varargin{i};
    if ischar(a)
        if brick.ismemberstr(a,{'char' 'hexa' 'HEXA' 'num'})
            outtype = a;
        else
            meth = a;
        end
