function out = savefig(varargin)
%SAVEFIG Save figure with a number of options
%---
% function savig([hobj][,fname][,options...])
% function im = savig([hobj][,options...])
%---
...
if nargin==0 || (nargin==1 && all(brick.isfigurehandle(varargin{1})))
    hfig = brick.switch_case(nargin,0,[],1,varargin{1});
    doax = false;
    s = struct( ...
    ...
    while k<length(varargin)
        ...
        elseif isscalar(a) && ishandle(a) && strcmp(get(a,'type'),'axes')
            ...
            hfig = brick.parentfigure(ha);
        elseif isempty(hfig) && ~doax && all(brick.isfigurehandle(a))
            hfig = a;
        ...
        elseif isnumeric(a) && isscalar(a)
            ...
            if ~brick.isfigurehandle(s.scaling), error 'numeric argument seems to be neither a figure handle, neither a scaling parameter', end
            hfig = [hfig s.scaling]; %#ok<AGROW>
            s.scaling = a;
        elseif isnumeric(a) && isvector(a) && length(a) == 4
            s.subframe = a;
        elseif iscell(a)
            if brick.ismemberstr(a{1},{'png' 'bmp' 'jpg' 'svg' 'eps' 'ps' 'pdf' 'fig'})
                s.format = lower(a);
            else
                fname = a;
            end
        elseif ischar(a)
            if ~isvector(a)
                fname = cellstr(a);
            elseif brick.ismemberstr(a,{'scale' 'scaling'})
                k = k+1;
...
elseif s.autoname
    fname = cell(1,nfig);
    for k=1:nfig, fname{k} = [brick.autofigname(hfig(k)) '_scale' num2str(s.scaling)]; end
else
    fname = cell(1,nfig);
    for k=1:nfig
        fname{k} = brick.savefile( ...
...
for k=1:nfig
    ...
    switch method
        case 'savefig'
            ...
            pos = brick.getpos(hfk,'inches'); % position in the screen
            ...
            set(hfk,'inverthardcopy',brick.onoff(invertcolor))
            printflags = brick.switch_case(invertcolor,{},{'-loose'});
            for i=1:length(formatk)
                fnamei = [fbasek '.' formatk{i}];                              % [new: pdf direct] 
                formatki = brick.switch_case(formatk{i},{'eps' 'ps'},'psc2',formatk{i}); % [new: pdf direct]
                if strcmp(formatki,'fig')
                    ...
                    if strcmp(formatk{i},'ps') && any(strfind(s.append,'pdf')) % [new: pdf direct] 
                        ps2pdf('psfile',[fbasek '.ps'],'pdffile',[fbasek '.pdf'], ...
                            'gspapersize',brick.strcat(paperpos(3:4),'x'),'deletepsfile',1,'verbose',0)
                    end
                end
            end
        ...
        case 'capture'
            ...
            elseif strcmp(s.subframe,'select sub-rectangle')
                rect = {brick.figselection(hfk)};
            elseif doax
                rect = {brick.pixelpos(ha,'recursive','strict')};
            end
            im = getfield(getframe(hfk,rect{:}),'cdata');
            if strcmp(s.subframe,'remove white sides')
                bg = im(1,1,:);
                isbg = all(brick.eq(im,bg),3);
            ...
            if dosavefile
                ...
                switch formatk
                    case 'show'
                        ...
                        hfnew = figure; brick.setfigsize(hfnew,nx,ny);
                        axes('pos',[0 0 1 1])
                        image(im)
                        set(gca,'xtick',[],'ytick',[])
                    case 'clipboard'
...
% store properties
state.controlsprop = brick.get(state.controls,'visible','struct');
...
state.figcallbacks = brick.get(hf,winprop,'struct');
state.butcallbacks = brick.get(state.buttons,{'callback' 'keypressfcn'},'struct');
state.speccallbacks = brick.get(state.spec,{'SelectionChangeFcn'},'struct');
state.allcallbacks = brick.get(state.allobj,{'userdata' 'buttondownfcn' 'createfcn' 'deletefcn' 'resizefcn' 'appdata'},'struct');

% overwrite properties
brick.set(hf,winprop,'default') % note that the default for 'closerequestfcn' is @closereq
brick.set(state.buttons,{'callback' 'keypressfcn'},'')
brick.set(state.spec,{'SelectionChangeFcn'},'')
brick.set(state.allobj,{'userdata' 'buttondownfcn' 'createfcn' 'deletefcn' 'resizefcn' 'appdata'},{[] '' '' '' '' struct})

%---
function restorefig(state)

brick.set(state.controls,state.controlsprop)

if ~isfield(state,'allobj'), return, end

brick.set(state.hf,state.figcallbacks)
brick.set(state.buttons,state.butcallbacks);
brick.set(state.spec,state.speccallbacks)
brick.set(state.allobj,state.allcallbacks)
