function [hl y] = spikedisplay(times,varargin)
%SPIKEDISPLAY Display events as small bars
%---
% function hl = spikedisplay(times[,ypos][,spikeheight [spikewidth]][,'numbers'],plot options...)
% function hl = spikedisplay(times,[ymin; ymax][,spikewidth][,'numbers'],plot options...)
%---
...
% - hl      handle of line(s)
%
% See also brick.rasterplot

% Thomas Deneux
% Copyright 2012-2017

if nargin==0, help brick.spikedisplay, return, end
...
if isempty(y)
    if ~isvector(times)
        [n1 n2] = size(times); 
        y = brick.add(-.4+(0:n1-1)'/n1*.7,1:n2);
...
if ~isempty(spikewidth)
    pixwidth = brick.coordinates(ha,'a2b',[spikewidth 0],'vector');
    pixwidth = pixwidth(1);
    hlok = hl(hl~=0);
...
elseif donumbers 
    ...
    spikewidth = brick.coordinates(ha,'b2a',[pixwidth 0],'vector');
