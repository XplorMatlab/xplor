...
% function poly = mouse([axes handle],'point|cross|poly|free|ellipse'[,msg])
% function [x y] = mouse([axes handle],'point|cross'[,msg])
% function rect = mouse([axes handle],'rect'[,msg])
% function [center axis e] = mouse([axes handle],'ellipse'[,msg])
% function [center axis e relradius] = mouse([axes handle],'ring'[,msg])
%---
...
% 
% See also brick.maskselect, interactivePolygon
...
switch type
    ...
    case {'line' 'segment'}
        ...
        data = brick.buttonmotion({@drawline,ha,hl,p1},hf,'doup');
        delete(hl(2))
        if showselection
            set(hl(1),'color','y')
        else
    ...
    case {'1D'}
        ...
        if strcmp(mode,'1D vertical')
            ...
            data = brick.buttonmotion({@draw1Dvertical,ha,hl,p1},hf,'doup');
            ...
            data = brick.buttonmotion({@draw1Dhorizontal,ha,hl,p1},hf,'doup');
    ...
    case {'rect' 'rectax' 'rectaxp' 'rectangle' 'xsegment' 'ysegment'}
        ...
        mode = brick.switch_case(type,'xsegment','x','ysegment','y','');
        rect = brick.buttonmotion({@drawrectangle,ha,hl,p0,mode},hf,'doup');
        delete(hl)
    ...
    case 'free'
        ...
        brick.buttonmotion({@freeform,ha,hl},hf)
    ...
    case {'ellipse' 'ring'}
        ...
        hl(1) = line(p(1,1),p(1,2),'color','k','linestyle','-','parent',ha);
        hl(2) = line(p(1,1),p(1,2),'color','w','linestyle',':','parent',ha);
        info = brick.pointer('flag','init');
        % circle
        if doscreencircle
            sz = brick.pixelsize(ha);
            ax = axis(ha); 
        ...
        brick.buttonmotion({@drawellipse,ha,hl,info,yadjust},hf,'doup')
        % make it an ellipse
        if strcmp(info.flag,'width')
            % change eccentricity -> ellipse
            brick.buttonmotion({@drawellipse,ha,hl,info,yadjust},hf)
        end
...
set(ha,'Tag','brick.mouse') % pour bloquer brick.imvalue !
...
state.hittest   = brick.get(state.obj,'hittest');
...
brick.set(state.obj,'hittest','off')
...
brick.set(state.obj,'hittest',state.hittest);
hf = state.hf;
...
switch flag
    case 'init'
        ...
        info.relradius = [];
        info.flag = 'axis';
        set(brick.parentfigure(ha),'windowbuttondownfcn',@(hf,evnt)set(info,'flag','click'))
    ...
    case 'click'
        ...
        p = ax(:,1) + u + v;
        p0 = brick.coordinates(ha,'a2s',p,'position');
...
p = get(ha,'currentpoint'); p=p(1,1:2);
dd = brick.coordinates(ha,'b2a',[9 9; 3 -12],'vector');
if isempty(msg)
    t = [];
else
    for i=1:2
        t(i) = text('parent',ha,'string',msg, ...
            ...
            'color',brick.switch_case(i,1,'k',2,'w')); %#ok<AGROW>
    end
end
