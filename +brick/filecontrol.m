function hu = filecontrol(varargin)
%FILECONTROL Select a file
%---
% function hu = filecontrol([hu,]'property1',value1,...)
%---
...
if ~brick.ismemberstr(mode,'get,save,dir')
...
switch mode
    case 'get'
        if exist(str,'file')
            fname = brick.getfile(filter,'Select file',str);
        else
            fname = brick.getfile(filter,'Select file');
        end
    case 'save'
        if ~isempty(str)
            fname = brick.savefile(filter,'Select file',str);
        else
            fname = brick.savefile(filter,'Select file');
        end
    case 'dir'
        if exist(str,'dir')
            fname = brick.getdir('Select directory',str);
        else
            fname = brick.getdir('Select directory');
        end
end
if ~fname, return, end
set(u,'string',fname)
if ~strcmp(fname,str), brick.evalcallback(get(u,'callback'),u,e), end
