...
% function x = get(h,f[,'cell|struct'])
% function x = get(h,f1,f2,...)
% function [x1 x2 ...] = get(...)
%--- 
...
% 
% brick.get also has some heuristics to get properties that do not exist:
...
if nargin==0, help brick.get, return, end

% Input
if nargin==2
    f = cellstr(varargin{1});
    flag = brick.switch_case(isscalar(f),'cell','struct');
elseif nargin==3 && brick.ismemberstr(varargin{2},{'cell' 'struct' 'char'})
    f = cellstr(varargin{1});
    flag = varargin{2};
else
    f = varargin;
...
for i=1:nobj
    for j=1:nfield
        ...
        elseif brick.ismemberstr(f{j},{'xdata' 'ydata' 'zdata'}) &&  strcmp(get(h(i),'type'),'text')
            ...
            pos = get(h(i),'pos');
            x{i,j} = pos(brick.switch_case(f{j}(1),'x',1,'y',2,'z',3));
...
if nargout>1
    ...
    switch flag
        ...
        otherwise
            error 'unknown flag'
            %     case 'array'
            %         if ~isscalar(x), error 'brick.get cannot output a string since more than one properties have been accessed', end
            %         x = x{1};
    end
    varargout = {x};
end
