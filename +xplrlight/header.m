classdef header < hgsetget
    ...
    properties (SetAccess='private')
        sublabels = xplrlight.dimensionlabel.empty(1,0);      % a dimensionlabel object
        label           % char
        n               % length
        categorical     % boolean
    ...
    methods
        function H = header(varargin)
            ...
            if ismultiple
                for i=1:nargin
                    H(i) = xplrlight.header(varargin{i}{:}); %#ok<AGROW>
                end
                return
            end
...
%                 end
                ...
                H.n = size(table,1);
                % set sublabels
                if isa(lab,'xplrlight.dimensionlabel')
                    H.sublabels = lab;
                else
                    ...
                    for i=1:nlabel
                        xi = table{1,i};
                        type = xplrlight.dimensionlabel.infertype(xi);
                        H.sublabels(i) = xplrlight.dimensionlabel(lab{i},type);
                    end
                end
                % set summary label
                if ~isempty(name)
                ...
                if isa(lab,'xplrlight.dimensionlabel')
                    ...
                    if ischar(varargin{2})
                        ...
                        if length(unit)>1 && unit(end)=='+'
                            ...
                            unit(end) = [];
                            [~, ~, measure] = xplrlight.bank.getunitinfo(unit);
                            if ~isempty(measure), unit = measure.units; end
                        end
                        H.sublabels = xplrlight.dimensionlabel(lab,'numeric',unit);
                    else
                        H.sublabels = xplrlight.dimensionlabel(lab,'numeric');
                    end
                end
...
%                 H.values = num2cell(H.start + (0:H.n-1)'*H.scale);
        ...
        function disp(H)
            str = 'xplrlight.header object with following info:';
            if ~isscalar(H)
                sz = size(H);
                    ...
                    case 'allunits'
                        if H.ismeasure
                            if isempty(H.allunits)
                                val{i} = '<invalid empty allunits>'; % old version of xplrlight.header class
                            else
    ...
    methods
        ...
        function newhead = addLabel(H,label,value)
            % xplrlight.dimensionlabel object
            if isa(label,'xplrlight.dimensionlabel')
                name = label.label;
                ...
                type = xplrlight.dimensionlabel.infertype(x);
                label = xplrlight.dimensionlabel(name,type);
            end
