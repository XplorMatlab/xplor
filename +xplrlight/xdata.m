classdef xdata < hgsetget
    ...
    properties (SetAccess='private')
        data        % ND array
        header = xplrlight.header.empty(1,0);
        name = '';
    end
    ...
    events
        ChangedData % sent with info xplrlight.eventinfo('data',chghead)
    end
    ...
    methods
        function x = xdata(dat,head,name)
            ...
            if nargin<2 || isempty(head)
                ...
                s = xplrlight.strictsize(dat);
                head = repmat({''},1,length(s));
            elseif ischar(head)
                head = {head};
            end
            if iscell(head)
                % create xplrlight.header objects from labels
                labels = head;
                head = xplrlight.header.empty(1,0);
                for i=1:length(labels)
                    if iscell(labels{i})
                        head(i) = xplrlight.header(labels{i}{:});
                    else
                        head(i) = xplrlight.header(labels{i},size(dat,i));
                    end
                end
            end
        ...
        function y = copy(x)
            y = xplrlight.xdata(x.data,x.header);
        end
        ...
        function chgData(x,data)
            ...
            datasz = xplrlight.strictsize(data,x.nd);
            ...
            x.data = data;
            if ~any(chgsz)
                %L3 notify(x,'ChangedData',xplrlight.eventinfo('data','chgdata'))
                return
            end
            ...
            %L3 notify(x,'ChangedData',xplrlight.eventinfo('data','chgdim',find(chgsz)))
        end
        function updateData(x,flag,dim,ind,value,newhead)
            ...
            if ~isequal(xplrlight.strictsize(newdata,length(newhead)),[newhead.n])
            ...
            x.data = newdata;
            % notification
            %L3 notify(x,'ChangedData',xplrlight.eventinfo('data',flag,dim,ind))
        end
        function updateDataDim(x,flag,dim,newdata,newhead)
            ...
            switch flag
                ...
                otherwise
                    ...
                    if ~isequal(xplrlight.strictsize(newdata,x.nd),x.sz)
                        error 'new data size does not match new header'
                    end
                    x.data = newdata;
            end
            % notification
            %L3 notify(x,'ChangedData',xplrlight.eventinfo('data',flag,dim))
        end
    end
