...
P=exch.ndSparse(Pf(:),size(Pf));   % =Pf
Q=exch.ndSparse(Qf(:),size(Qf));   % =Qf
A=exch.ndSparse(Af);   % =Af
B=exch.ndSparse(Bf);   % =Bf
C=exch.ndSparse(Cf);   % =Cf
S=exch.ndSparse(Sf);   % =Sf
Vc=exch.ndSparse(Vcf);   % =Vcf
Vr=exch.ndSparse(Vrf);   % =Vrf
...
Error(end+1)= err( isempty(exch.ndSparse([])),1);
...
Error(end+1)= err( isequalwithequalnans(Z,Zf), 1 );  %#ok<*DISEQN>
...
%Test of real, imag, conj, abs,sqrt
     ...
     Z=exch.ndSparse(Bf+Af); %complex result
     Zf=full(Z);
...
Error(end+1)= err( Zf(exch.ndSparse(idx)) , Zf(idx) );   
...
Error(end+1)= err( bsxfun(fun,Z,H) ,     bsxfun(fun,Zf,Hf)  );     %#ok<*AGROW>
...
Error(end+1)= err( feval(fun,Z,H) ,     bsxfun(fun,Zf,Hf)  );     %#ok<*AGROW>
Error(end+1)= err( feval(fun,Z,Hf) ,     bsxfun(fun,Zf,Hf)  );           
Error(end+1)= err( feval(fun,Zf,H) ,     bsxfun(fun,Zf,Hf)  );    
Error(end+1)= err( feval(fun,three,H) ,     bsxfun(fun,three,Hf)  );  
Error(end+1)= err( feval(fun,Zf,three) ,     bsxfun(fun,Zf,three)  ); 
...
Error(end+1)= err( feval(fun,Z,H) ,     bsxfun(fun,Zf,Hf)  );    
Error(end+1)= err( feval(fun,Z,Hf) ,     bsxfun(fun,Zf,Hf)  );           
Error(end+1)= err( feval(fun,Zf,H) ,     bsxfun(fun,Zf,Hf)  );    
Error(end+1)= err( feval(fun,onebit,H) ,     bsxfun(fun,onebit,Hf)  );  
Error(end+1)= err( feval(fun,Zf,onebit) ,     bsxfun(fun,Zf,onebit)  ); 
...
%Test of ndSparse.build, nzmax, nonzeros   
      ...
      nzm=10;
      Z=exch.ndSparse.build([1 1 1; 2 2 2; 3 3 3; 4 4 4], 1:4,[4 4 6],nzm);
...
Error(end+1)= err( nonzeros(Z) , nonzeros(Zf) );
      ...
      nzm=10;
      Z=exch.ndSparse.build([1 1 1; 2 2 2; 3 3 3; 4 4 4], 5,[4 4 6],nzm);
...
Error(end+1)= err( nonzeros(Z) , nonzeros(Zf) );
       ...
       Z=exch.ndSparse.build([4 4 6]);
       Zf=zeros(4,4,6); 

Error(end+1)= err( Z , Zf  );   
Error(end+1)= err( exch.ndSparse.build([],[],[4 4 6]) , Zf  );    
...
%Test of ndSparse.spalloc
      ...
      nzm=10;
      Z=exch.ndSparse.spalloc([3,5,3],nzm);
      Zf=full(Z);
...
%Test of ndSparse.accumarray
         ...
         Z=exch.ndSparse.accumarray(args{:});
         Zf=accumarray(args{:});
...
%Test of spones
          ...
          Z=exch.ndSparse.build([1,1,1], 5,[3,5,3]);
          Zf=full(Z); Zf(~~Zf)=1;
...
function errval=DiscrepancyMeasure(X,Y,TOL,CHECKTYPES)
  ...
  isndSparse=@(c) ~isempty(strfind(class(c),'exch.ndSparse'));
  
  if CHECKTYPES
    if ~isndSparse(X),
       warning(['X is not exch.ndSparse class, but rather class ' class(X)]) 
    end
  end
