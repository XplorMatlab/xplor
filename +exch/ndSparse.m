 ...
 methods
    ...
    % OBJ=ndSparse(A,[M,N,P,...]) is equivalent to reshape(ndSparse(A),[M,N,P,...]) 
     ...
     %reshape - same syntax as for full arrays
         ...
         obj=exch.ndSparse(obj,newshape);
    ...
    %subsref - same indexing rules as for full arrays
        ...
        if  other.boolConsol 
          ...
          objnew=exch.ndSparse(data,targetshape);
        ...
        if isequal(rhs,[])%NULL ASSIGNMENT
          ...
          N=length(S.subs);        
            ...
            if dim>nn,
                ...
                sz=trail1s(sz,dim);
                sz(end)=0;
                objnew=exch.ndSparse.build(sz);
                return; 
                
            end
            
            
           if exch.ndSparse.oldstyle%memory liberal style  
                ...
                if isequal(idx,':') || ( sz(dim)==1 && ~isempty(idx) )
                  ...
                  sz(dim)=length(T);
                  objnew=exch.ndSparse.build(ndSubs , vals, sz);  
        ...
        elseif isempty(rhs)   
         ...
         [E,targetshape,boolGrow,other]=nd2matrixIndex(S.subs,obj.ndShape,'subsasgn');
          ...
          if boolGrow
              ...
              objnew=exch.ndSparse.build(ndSubs,vals,targetshape,nzmax(obj));
      ...
      [varargout{1:nargout}]=minEngine(1,obj,varargin{:});
       ...
       %vertcat - same syntax as for full arrays
         ...
         %permute - same syntax as for full arrays
           ...
           objnew=exch.ndSparse.build(ndSubs,vals,newshape,nzmax(obj));
...
%              obj = finalObject( mkCompat(L,s) | mkCompat(R,s),s);
       ...
       %inv - same behavior as for full arrays
        ...
        if ndims(obj)>2, error 'Operation transpose defined for 2D arrays only'; end 
          ...
          obj = exch.ndSparse(inv(sparse2d(obj)));
       ...
       %triu
        ...
        if ndims(obj)>2, error 'Operation transpose defined for 2D arrays only'; end 
          ...
          obj = exch.ndSparse(  triu(  sparse2d(obj) , varargin{:} )  );
        ...
        if ndims(obj)>2, error 'Operation transpose defined for 2D arrays only'; end 
          ...
          obj = exch.ndSparse( tril(  sparse2d(obj) , varargin{:} ));
       ...
       %transpose - same behavior as for full arrays
          ...
          obj = exch.ndSparse(sparse2d(obj).');
    ...
    %repmat - same syntax as for full arrays
        ...
        if ~all(repDims)%some repDims are 0
           objnew=exch.ndSparse([],newshape); return
        end  
         ...
         if isempty(vals)
             objnew=exch.ndSparse.build(newshape); return 
         end
         ...
         entrytable=[ndSubs{:},vals(:)];
           ...
           objnew=exch.ndSparse.build(entrytable(:,1:end-1),...
                                 entrytable(:,end),...
                                 newshape);     
    ...
    %bsxfun - same syntax as for full arrays
        ...
        isL=isa(L,'exch.ndSparse');
        isR=isa(R,'exch.ndSparse');
        ...
        if isL && isR
         ...
         out=exch.ndSparse(bsxfun(fun,L.data,R.data) , newshape);
         return
        ...
        elseif isL
             ...
             if isempty(Rsamp), Rsamp=reshape(Rsamp,0,1); else Rsamp=Rsamp(1); end
              ...
              if isOutFull%cheapest to do everything as full array operations
                ...
                out=exch.ndSparse( data , newshape(order));
    ...
    %flipdim - same behavior as for full arrays
       ...
       objnew=exch.ndSparse.build([ndSubs{:}], vals, ndShape,nzm);
    ...
    %MATLAB convn method.
         ...
         switch truncType
            ...
            case 'valid'
                ...
                if any(lb>ub) %result will be empty
                    ...
                    obj=exch.ndSparse.build([],[],convShape );
                    return
                end
       ...
       obj=exch.ndSparse.build(convCoords,convVals,convShape);
   ...
   methods (Hidden)
      ...
      function ndShape=getndShape(obj) 
       ...
       %sumEngine
         ...
         memliberal=memliberal | exch.ndSparse.oldstyle;  
           
         if ndims(obj)<=2 %Matrix case
             objnew=exch.ndSparse( sum(obj.data, varargin{:}) );
             return;
         end
            ...
            if memliberal
                ...
                sz(dim)=1;
                 ...
                 objnew=exch.ndSparse.accumarray([ndSubs{:}],vals,sz);           
       ...
       %anyEngine 
         ...
         if ndims(obj)<=2 %Matrix case
             objnew=exch.ndSparse( any(obj.data, varargin{:}) );
             return;
         end
       ...
       %allEngine
         ...
         if ndims(obj)<=2 %Matrix case
             objnew=exch.ndSparse( all(obj.data, varargin{:}) );
             return;
         end
       ...
       %meanEngine  
         ...
         if ndims(obj)<=2 %Matrix case
             objnew=exch.ndSparse( mean(obj.data, varargin{:}) );
             return;
         end
    ...
    %circshiftEngine 
       ...
       if memliberal 
           ...
           circDims=circDims(firstShift:end);
            ...
            objnew=exch.ndSparse(data, initialshape);
         ...
         objnew=exch.ndSparse.build(ndSubs,vals,sz,nzmax(obj));
       ...
       %catEngine - same syntax as for full arrays
         ...
         memliberal=memliberal | exch.ndSparse.oldstyle;  
          ...
          map=cellfun('isclass',varargin,'exch.ndSparse');
          first=find(map,1); 
          if first==1
             L=varargin{first};
         ...
         if dim>=ndimsL || (ndimsL<=2) %equivalent to 2D cat
            ...
            objnew=exch.ndSparse( cat(min(dim,2),L,R),newshape);
           ...
           if memliberal
            ...
            R=ExtractDim1Reshaping(R,dim,szR);
             ...
             objnew=exch.ndSparse(L,newshape);
            
           else
              
               if ~map(1), L=exch.ndSparse(L); end
               if ~map(2), R=exch.ndSparse(R); end              
               ...
               objnew=exch.ndSparse.build( [[LSubs{:}];[RSubs{:}]] , [Lvals;Rvals] , newshape);
         ...
         if isempty(R)
          ...
          if memliberal  
                ...
                varargout{1}=invExtractDim1Reshaping(varargout{1},dim,sz);
                  varargout{1}=exch.ndSparse(varargout{1},sz); 
  ...
  methods (Static)
     ...
     function obj=loadobj(B)
         ...
         obj=exch.ndSparse(B.data,B.ndShape);
     ...
     %See also ndSparse.spalloc, ndSparse.sprand, ndSparse.sprandn
         ...
         switch nargin
             ...
             case 1  %ndSparse.build(ndShape)
                 ...
                 data=sparse(M,N);
                 obj=exch.ndSparse(data,ndShape);
     ...
     %out: output all-zero ndSparse object of dimensions ndShape 
         ...
         obj=exch.ndSparse.build([],[],ndShape,nzmax);
     ...
     %out: output ndSparse object of dimensions ndShape 
         ...
         obj=exch.ndSparse( sprand(mm,nn,density) , ndShape);
     ...
     %out: output ndSparse object of dimensions ndShape 
         ...
         obj=exch.ndSparse( sprandn(mm,nn,density) , ndShape);
     ...
     %sensible FILLVAL is zero. 
         ...
         if isempty(subs)
            obj=exch.ndSparse.build(sz); return 
         end
         ...
         obj=exch.ndSparse(data,desiredShape);
...
%invExtractDim1Reshaping - not a precise inverse, only up to a reshaping
    ...
    %obj=exch.ndSparse(data,newshape);
...
%invExtractDimNReshaping
     ...
     if dim<=nd
       ...
       order=[1:dim-1, dim+1:nd, dim];
       obj=exch.ndSparse(data,newshape(order));
